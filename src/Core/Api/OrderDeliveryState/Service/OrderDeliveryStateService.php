...
class OrderDeliveryStateService {
	...
	protected function getHoldStateId(string $stateMachineId, Context $context): string
	{
		...
		if (is_null($holdStateMachineStateEntity)) {
			$translations = $this->localeCodeProvider->getAvailableTranslations('trustpayments.deliveryState.hold', 'Hold', $context);
			$data         = [
				'id'             => $holdStateId,
	...
	{
		$translations = $this->localeCodeProvider->getAvailableTranslations('trustpayments.deliveryState.unhold','Unhold',$context);
